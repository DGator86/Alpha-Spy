...
class BreadthAggregator:
    ...
    def aggregate(
        ...
        *,
        ...
        pct_struct_bull = _fraction([None if _struct(item) is None else _struct(item) > 0 for item in constituents])
        pct_struct_bear = _fraction([None if _struct(item) is None else _struct(item) < 0 for item in constituents])
        absorption_values = [item.flow.absorption for item in constituents if item.flow.absorption is not None]
        absorption_pairs = [(item.flow.absorption, item.weight) for item in constituents if item.flow.absorption is not None]
        ...
        for sector, items in sorted(sector_map.items()):
            sectors.append(
                SectorFactors(
                    ...
                    participation=_mean([1.0 if (item.return_5m or 0.0) > 0 else -1.0 for item in items if item.return_5m is not None]),
                )
            )
        ...
        return MarketFactors(
            ...
            pct_positive_cvd=_fraction([None if item.auction.cvd_session is None else item.auction.cvd_session > 0 for item in constituents]),
            pct_buy_absorption=_fraction([None if item.flow.buy_absorption is None else item.flow.buy_absorption > 0.5 for item in constituents]),
            pct_sell_absorption=_fraction([None if item.flow.sell_absorption is None else item.flow.sell_absorption > 0.5 for item in constituents]),
            pct_bullish_sweep=_fraction([None if item.structure.sweep_low_score is None else item.structure.sweep_low_score > 0 for item in constituents]),
            pct_bearish_sweep=_fraction([None if item.structure.sweep_high_score is None else item.structure.sweep_high_score > 0 for item in constituents]),
            ...
            pct_breaking_highs=_fraction([None if item.structure.structure_break_strength is None else item.structure.structure_break_strength > 0 for item in constituents]),
            pct_breaking_lows=_fraction([None if item.structure.structure_break_strength is None else item.structure.structure_break_strength < 0 for item in constituents]),
