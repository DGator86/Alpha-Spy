...
# Long-running services managed by spy-der.target. The oneshot units (backup,
...
SUITE_SERVICES = [u for u in SERVICE_UNITS if "User=spyder" in u.read_text(encoding="utf-8")]
...
def _shell_scripts() -> list[Path]:
    return sorted([*SCRIPTS.glob("*.sh"), SCRIPTS / "spy-der-drive-backup", REPO_ROOT / "install.sh"])
...
def test_yaml_scripts_use_the_suite_interpreter(script):
    ...
    These scripts edit /etc/spy-der/config.yaml. Invoking bare `python3` meant
    ...
    """
    text = script.read_text(encoding="utf-8")
    assert "/opt/spy-der/venv/bin/python" in text, (
...
def test_installer_preserves_existing_data_roots():
    ...
    text = (SCRIPTS / "install_vps.sh").read_text(encoding="utf-8")
    for protected in ("/var/lib/spy-der", "/var/lib/zerodte"):
...
def test_uninstall_preserves_data_and_configuration():
    ...
    for protected in ("/var/lib/spy-der", "/var/lib/zerodte", "/etc/spy-der"):
        assert not re.search(rf"rm\s+-rf?\s+{re.escape(protected)}\b", text), (
...
def test_long_running_services_belong_to_the_target(unit):
    """Stopping spy-der.target must stop the session services with it."""
    text = unit.read_text(encoding="utf-8")
    assert "PartOf=spy-der.target" in text, f"{unit.name} will not stop with the target"
...
def test_target_pulls_in_every_long_running_service():
    ...
    wants = (SYSTEMD / "spy-der.target").read_text(encoding="utf-8")
    for unit in LONG_RUNNING:
        assert unit.name in wants, f"spy-der.target does not pull in {unit.name}"
...
def test_units_launch_only_installed_binaries(unit):
    ...
    installed = {"/opt/spy-der/venv/bin/spy-der", "/usr/local/sbin/spy-der-drive-backup"}
...
def test_apis_bind_to_loopback_only():
    ...
    decision = (REPO_ROOT / "systemd" / "spy-der-decision.service").read_text(encoding="utf-8")
...
def _doctor(tmp_path: Path, **universe) -> tuple[int, dict]:
    ...
    config["paths"] = {
        ...
        "database": f"{state}/journal/suite-v2.db",
    ...
    result = subprocess.run(
        [sys.executable, "-m", "spy_der.cli", "--config", str(path), "--state-root", str(state), "doctor"],
        capture_output=True,
        text=True,
        cwd=REPO_ROOT,
