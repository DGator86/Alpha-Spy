...
REGIMES = (
    Regime("low_vol", 0.35, 0.115, 0.23, 0.00035, 0.004),
    Regime("normal_vol", 0.30, 0.165, 0.32, 0.00020, 0.008),
    Regime("elevated_vol", 0.25, 0.245, 0.47, -0.00035, 0.025),
    Regime("crisis_vol", 0.08, 0.400, 0.68, -0.00150, 0.100),
    Regime("extreme_vol", 0.02, 0.720, 0.84, -0.00600, 0.350),
)
...
MISPRICING_PROBABILITIES = np.array([0.82, 0.06, 0.06, 0.04, 0.02])
...
def adaptive_directional_credit_specs(
    *,
    spot: float,
    tenor: float,
    market_iv: float,
    ...
    """One-sided, physical-quantile credit spreads with bounded tail loss.

    These are designed to raise win frequency without using naked shorts. The short
    strike is located from the independently estimated physical terminal distribution,
    while direction, breadth, and concentration determine whether the setup is eligible.
    """
    if entry_minute < 150 or entry_minute > 335:
        return []
    physical_move = max(model_p_iv * math.sqrt(max(tenor, 1e-12)), 1e-6)
    directional_ratio = forecast_remaining_return / physical_move
    iv_overpricing = (market_iv - model_p_iv) / max(market_iv, 1e-6)
    if iv_overpricing < 0.035 or abs(directional_ratio) < 0.075:
        return []
    if concentration_5 > 0.72:
        return []

    normal = NormalDist()
    mu = math.log(spot) + (model_p_drift_annual - 0.5 * model_p_iv**2) * tenor
    scale = model_p_iv * math.sqrt(max(tenor, 1e-12))
    # Stronger confirmation permits a slightly closer short strike, but the target
    # remains a high-probability one-sided structure.
    tail_probability = float(np.clip(0.105 - 0.035 * min(abs(directional_ratio), 0.8), 0.065, 0.105))
    wing = 1.0 if model_p_iv >= 0.30 else 2.0
    specs = []
    if directional_ratio > 0 and breadth_5 >= 0.53:
        short_put = float(math.floor(math.exp(mu + scale * normal.inv_cdf(tail_probability))))
        short_put = min(short_put, math.floor(spot) - 1.0)
        long_put = max(1.0, short_put - wing)
        specs.append((
            "ADAPTIVE_BULL_PUT_CREDIT_SPREAD",
            "adaptive_directional_credit",
            "credit",
            [("P", long_put, 1), ("P", short_put, -1)],
            0.92,
            abs(directional_ratio),
            iv_overpricing,
        ))
    elif directional_ratio < 0 and breadth_5 <= 0.47:
        short_call = float(math.ceil(math.exp(mu + scale * normal.inv_cdf(1.0 - tail_probability))))
        short_call = max(short_call, math.ceil(spot) + 1.0)
        long_call = short_call + wing
        specs.append((
            "ADAPTIVE_BEAR_CALL_CREDIT_SPREAD",
            "adaptive_directional_credit",
            "credit",
            [("C", short_call, -1), ("C", long_call, 1)],
            0.92,
            abs(directional_ratio),
            iv_overpricing,
        ))
...
def build_strategy_candidates(
    *,
    ...
    specs.extend(adaptive_directional_credit_specs(
        spot=spot,
        tenor=tenor,
        market_iv=market_iv,
        model_p_iv=model_p_iv,
    ...
    for name, family, premium_style, legs, uncertainty_scale, tail_risk_score, range_score in specs:
        ...
        if family == "directional_credit" and vol_gap > 0.006:
            continue
        if family == "adaptive_directional_credit" and market_iv <= model_p_iv:
            continue
        ...
        if premium_style == "debit" and candidate.entry_cashflow <= 0.0:
            continue
        if family == "adaptive_directional_credit":
...
def candidate_passes(candidate: StructureQuote) -> bool:
    ...
    if candidate.physical_expected_net / max(max_loss, 1e-9) < 0.010:
        return False
    if candidate.family == "adaptive_directional_credit":
        credit = -candidate.entry_cashflow
        reward_to_risk = credit / max(max_loss, 1e-9)
        return bool(
            credit >= 0.035
            and max_loss <= 2.25
            and reward_to_risk >= 0.075
            and candidate.net_q_edge >= -0.030
            and candidate.physical_expected_net >= 0.010
            and candidate.predicted_probability_profit >= 0.72
            and candidate.tail_risk_score >= 0.075
            and candidate.range_score >= 0.035
        )
    ...
    if actual_style == "credit":
        credit = -candidate.entry_cashflow
        reward_to_risk = credit / max(max_loss, 1e-9)
        physical_score = candidate.physical_expected_net / max(candidate.uncertainty, 1e-9)
        return bool(
            credit >= 0.025
            and reward_to_risk >= 0.055
            and candidate.net_q_edge >= -0.025
            and candidate.physical_expected_net >= 0.012
            and physical_score >= 0.08
            and candidate.predicted_probability_profit >= 0.68
        )
...
def signal_state_at(
    ...
    *,
    ...
    """Compute the same observable constituent state used at entry and after entry."""
    remaining_minutes = MINUTES_PER_DAY - minute
    start_5 = max(0, minute - 4)
    start_15 = max(0, minute - 14)
    cumulative_5 = constituent_returns[start_5 : minute + 1].sum(axis=0)
    cumulative_15 = constituent_returns[start_15 : minute + 1].sum(axis=0)
    ...
    forecast_per_minute = (
        0.11 * pressure_5 / max(len(cumulative_5), 1)
        + 0.055 * pressure_15 / max(len(cumulative_15), 1)
        + 0.035 * equal_weight_5 / max(len(cumulative_5), 1)
    )
    ...
    return SignalState(
        forecast_return=forecast_remaining_return,
        breadth=breadth_5,
...
def adaptive_credit_liquidation(
    candidate: StructureQuote,
    *,
    entry_minute: int,
    ...
    profit_target = max(0.015, 0.35 * credit)
    stop_loss = min(0.28 * candidate.max_loss, max(1.25 * credit, 0.12 * candidate.max_loss))
    short_leg = next((right, strike) for right, strike, quantity in candidate.legs if quantity < 0)
    right, short_strike = short_leg
    long_strike = next(strike for leg_right, strike, quantity in candidate.legs if quantity > 0 and leg_right == right)
    wing_width = abs(short_strike - long_strike)
    threat_buffer = 0.20 * max(wing_width, 1.0)
    ...
    for minute in range(entry_minute + 5, default_exit_minute + 1, 5):
        ...
        threatened = spot <= short_strike + threat_buffer if right == "P" else spot >= short_strike - threat_buffer
        if threatened and pnl < 0.0:
            return cashflow, minute, spot, "pre_strike_tail_exit"
        if minute >= entry_minute + 20 and market_iv[minute] - model_q_iv[minute] < -0.002 and pnl >= 0.005:
...
def create_universe(rng: np.random.Generator, constituents: int = 50, sectors: int = 11):
    tickers = np.array([f"S{i:03d}" for i in range(constituents)])
    raw_weights = rng.lognormal(mean=0.0, sigma=0.55, size=constituents)
    raw_weights.sort()
...
def simulate_world(
    *,
    ...
    episode_start = int(rng.integers(40, 320))
    episode_duration = int(rng.integers(10, 61))
    episode_end = min(MINUTES_PER_DAY - 10, episode_start + episode_duration)
    episode_strength = float(rng.uniform(0.02, 0.10))

    event_type = str(rng.choice(
        ["none", "broad_up", "broad_down", "sector_up", "sector_down", "mega_up", "mega_down", "macro_up", "macro_down"],
        p=[0.58, 0.05, 0.05, 0.02, 0.02, 0.07, 0.07, 0.07, 0.07],
    ))
    if event_type.startswith("mega"):
        event_start = int(rng.integers(5, 71))
        event_duration = int(rng.integers(8, 31))
        event_amplitude = float(rng.uniform(0.0015, 0.0060))
    elif event_type.startswith("macro"):
        event_start = int(rng.choice([30, 270, 300, 360]) + rng.integers(-4, 5))
        event_start = int(np.clip(event_start, 10, 370))
        event_duration = int(rng.integers(6, 26))
        event_amplitude = float(rng.uniform(0.0025, 0.0120))
    elif event_type.startswith("sector"):
        event_start = int(rng.integers(30, 301))
        event_duration = int(rng.integers(12, 46))
        event_amplitude = float(rng.uniform(0.0015, 0.0065))
    else:
        event_start = int(rng.integers(25, 326))
        event_duration = int(rng.integers(10, 41))
        event_amplitude = float(rng.uniform(0.0015, 0.0075))

    # Regime-dependent dynamic correlation and volatility.
    base_corr = regime.average_correlation
    if regime.name in {"crisis_vol", "extreme_vol"} or (regime.name == "elevated_vol" and rng.random() < 0.18):
        corr_shock_start = int(rng.integers(70, 301))
    else:
    ...
    for minute in range(MINUTES_PER_DAY):
        ...
        if minute >= corr_shock_start:
            ...
            annual_vol *= 1.18
        if regime.name in {"elevated_vol", "crisis_vol", "extreme_vol"} and market_state < -0.5:
            corr = min(0.94, corr + 0.10)

        minute_index_sigma = annual_vol / math.sqrt(MINUTES_PER_YEAR) * tod[minute]
        # Standardized Student-t shocks reproduce empirically heavy intraday tails.
        market_state = 0.22 * market_state + rng.standard_t(5) / math.sqrt(5.0 / 3.0)
        sector_state = 0.12 * sector_state + rng.standard_t(7, size=sectors) / math.sqrt(7.0 / 5.0)
        idio = rng.standard_t(6, size=constituents) / math.sqrt(6.0 / 4.0)
        ...
        if event_start <= minute < event_start + event_duration:
            ...
            if event_type.startswith("broad"):
                ...
                minute_returns += per_minute_impulse * (0.90 + 0.35 * beta)
                # Macro surprises temporarily increase co-movement.
                minute_returns += per_minute_impulse * 0.25

        # Occasional jump, especially in the jump regime.
        if rng.random() < regime.jump_probability / MINUTES_PER_DAY:
            jump = rng.standard_t(5) * 0.0045 + (-0.0045 if regime.name in {"crisis_vol", "extreme_vol"} else -0.0010)
    ...
    for minute in range(MINUTES_PER_DAY):
        ...
        if minute > 0:
            # Efficient SPY/SPX option markets assimilate basket information rapidly.
            # The prior five-minute artificial lag is deliberately removed.
            raw_market_iv[minute] = (
                0.80 * raw_market_iv[minute - 1]
                + 0.20 * constituent_q_iv[max(0, minute - 1)]
                + rng.normal(0.0, 0.0008)
            )
            raw_market_skew[minute] = (
                0.88 * raw_market_skew[minute - 1]
                + 0.12 * model_q_skew[max(0, minute - 2)]
    ...
    for minute in range(min_entry_minute, max_entry_minute + 1, 5):
        ...
        spot = float(spy_prices[minute])
        entry_state = signal_state_at(
            minute,
            constituent_returns=constituent_returns,
            weights=weights,
        )
        forecast_remaining_return = entry_state.forecast_return
        breadth_5 = entry_state.breadth
        concentration = entry_state.concentration
        ...
        for candidate in candidates:
            ...
            if not candidate_passes(candidate):
                continue
            if candidate.premium_style == "credit":
                physical_score = candidate.physical_expected_net / max(candidate.uncertainty, 1e-9)
                selection_score = (
                    physical_score
                    + 0.28 * candidate.predicted_probability_profit
                    + 0.14 * min((-candidate.entry_cashflow) / max(candidate.max_loss, 1e-9), 1.0)
                    + 0.08 * min(candidate.physical_expected_net / max(candidate.max_loss, 1e-9), 1.0)
                    - 0.06 * max(-candidate.net_q_edge, 0.0) / max(candidate.uncertainty, 1e-9)
                )
            else:
                selection_score = (
                    candidate.score
    ...
    for candidate, metadata in best_by_strategy.values():
        professional_exit = professional_liquidation(
            ...
            entry_spot=float(metadata["spot_entry"]),
            entry_forecast_return=float(metadata["forecast_remaining_return"]),
            entry_breadth=float(metadata["breadth_5"]),
            entry_model_p_iv=float(metadata["model_p_iv"]),
            minutes_per_year=MINUTES_PER_YEAR,
            spy_prices=spy_prices,
            market_iv=market_iv,
            market_skew=market_skew,
            model_q_iv=model_q_iv,
            signal_at=lambda m: signal_state_at(
                m,
                constituent_returns=constituent_returns,
                weights=weights,
            ),
        ...
        exit_cashflow = professional_exit.cashflow
        trade_exit_minute = professional_exit.minute
        trade_exit_spot = professional_exit.spot
        exit_reason = professional_exit.reason
        ...
        trade = {
            ...
            "professional_managed": True,
            "holding_minutes": professional_exit.holding_minutes,
            "mfe": professional_exit.mfe * MULTIPLIER,
            "mae": professional_exit.mae * MULTIPLIER,
            "exit_forecast_return": professional_exit.final_forecast_return,
            "exit_breadth": professional_exit.final_breadth,
            "exit_iv_gap": professional_exit.final_iv_gap,
...
def build_summary(worlds: pd.DataFrame, trades: pd.DataFrame) -> dict:
    ...
    return {
        "simulation": {
            ...
            "seed": None,
            "empirical_profile": "empirical_occurrence_v1",
...
def write_report(
    ...
    lines = [
        "# Historically Anchored Defined-Risk Intraday Strategy Tournament",
        "",
        "## Scope",
        "",
        "- One-minute historically anchored market universes with decisions every five minutes",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the historically anchored defined-risk SPY stress tournament.")
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "reports" / "empirical_stress_tournament",
    )
