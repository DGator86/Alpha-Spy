...
def simulate_world(
    *,
    ...
    for minute in range(min_entry_minute, max_entry_minute + 1):
        ...
        # Promoted policy, learned from the prior 1,000-world run:
        #   * only the long straddle had demonstrated positive expectancy;
        #   * bear-regime straddles were suppressed;
        #   * low-concentration and overconfident/extreme-volatility candidates were suppressed;
        #   * bull, jump, and correlation-shock regimes receive a larger but capped risk allocation.
        promoted = [candidate for candidate in candidates if candidate.name == "LONG_STRADDLE"]
        if not promoted:
            continue
        best = max(promoted, key=lambda candidate: candidate.score)
        ...
        promoted_quality = (
            concentration >= 0.37
            and model_p_iv[minute] <= 0.60
            and best.predicted_probability_profit <= 0.75
            and best.score <= 0.85
        )
        if (
            promoted_quality
            and best.net_q_edge >= 0.08
            and best.physical_expected_net >= 0.08
            and best.score >= score_floor
            and best.predicted_probability_profit >= 0.52
            and best.max_loss > 0.0
            and best.max_loss <= 4.50
        ):
            contracts = 2 if regime.name in {"bull", "correlation_shock", "jump"} else 1
            if best.max_loss * MULTIPLIER * contracts > 900.0:
                contracts = 1
            ...
            trade = {
                ...
                "structure": best.name,
                "policy": "promoted_long_straddle_v1",
                "contracts": contracts,
                "spot_entry": spot,
                ...
                "max_loss": best.max_loss * MULTIPLIER * contracts,
                "terminal_payoff": terminal_payoff * MULTIPLIER * contracts,
                "pnl": pnl_per_share * MULTIPLIER * contracts,
...
def build_summary(worlds: pd.DataFrame, trades: pd.DataFrame) -> dict:
    ...
    return {
        "simulation": {
            ...
            "policy_status": "promoted from the original 1,000-world result: long straddles only, weak contexts suppressed, top regimes risk-weighted",
            "seed": None,
        },
...
def write_report(
    ...
    lines = [
        "# Promoted-Strategy 1,000-World, 1-Minute-Day Monte Carlo Report",
        "",
        "## Scope",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the promoted-strategy 1,000-world, 1-minute SPY constituent-alpha Monte Carlo day.")
    parser.add_argument("--worlds", type=int, default=1000)
    parser.add_argument("--seed", type=int, default=20260804)
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=ROOT / "reports" / "monte_carlo_1000_worlds_1m_promoted",
    )
